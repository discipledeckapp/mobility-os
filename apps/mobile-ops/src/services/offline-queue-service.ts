...
type OfflineAction =
  | {
      id: string;
      type: typeof OFFLINE_ACTION_TYPE.assignmentAccept;
      payload: { assignmentId: string; notes?: string };
      createdAt: string;
    }
  | {
      id: string;
      type: typeof OFFLINE_ACTION_TYPE.assignmentDecline;
      payload: { assignmentId: string; notes?: string };
      createdAt: string;
    }
  | {
...
async function executeAction(action: OfflineAction) {
  switch (action.type) {
    case OFFLINE_ACTION_TYPE.assignmentAccept:
      return acceptDriverAssignmentTerms(action.payload.assignmentId, action.payload.notes);
    case OFFLINE_ACTION_TYPE.assignmentDecline:
      return declineDriverAssignment(action.payload.assignmentId, action.payload.notes);
