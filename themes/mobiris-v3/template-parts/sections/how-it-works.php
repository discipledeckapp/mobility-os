...
$steps = [];
for ( $i = 1; $i <= 4; $i++ ) {
    $title = get_theme_mod( "mobiris_hiw_step_{$i}_title", $step_defaults[ $i ][0] );
    $body  = get_theme_mod( "mobiris_hiw_step_{$i}_body",  $step_defaults[ $i ][1] );
    if ( $title ) {
        $steps[] = [ 'num' => $i, 'title' => $title, 'body' => $body ];
    }
}
?>
...
<section id="how-it-works" class="hiw" aria-labelledby="hiw-headline">
    <div class="container">
        ...
        <div class="hiw__header">
            <?php if ( $label ) : ?>
                <p class="text-label hiw__label"><?php echo esc_html( $label ); ?></p>
            <?php endif; ?>
            <h2 id="hiw-headline" class="heading heading--h2"><?php echo esc_html( $headline ); ?></h2>
            <?php if ( $subtext ) : ?>
                <p class="hiw__subtext"><?php echo esc_html( $subtext ); ?></p>
            <?php endif; ?>
        </div>
        ...
        <div class="hiw__steps">
            <?php foreach ( $steps as $step ) : ?>
                <div class="hiw__step">
                    ...
                    <p class="hiw__step-body"><?php echo esc_html( $step['body'] ); ?></p>
                </div>
            <?php endforeach; ?>
        </div>
