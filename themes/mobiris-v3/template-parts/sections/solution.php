...
$features = [];
for ( $i = 1; $i <= 4; $i++ ) {
    $title = get_theme_mod( "mobiris_solution_feature_{$i}_title", $feature_defaults[ $i ][0] );
    $body  = get_theme_mod( "mobiris_solution_feature_{$i}_body",  $feature_defaults[ $i ][1] );
    if ( $title ) {
        $features[] = [ 'num' => sprintf( '%02d', $i ), 'title' => $title, 'body' => $body ];
    }
}
...
<section class="solution" aria-labelledby="solution-headline">
    <div class="container">
        ...
        <div class="solution__header">
            <?php if ( $label ) : ?>
                <p class="text-label solution__label"><?php echo esc_html( $label ); ?></p>
            <?php endif; ?>
            <h2 id="solution-headline" class="heading heading--h2 solution__headline">
                <?php echo esc_html( $headline ); ?>
            </h2>
            <?php if ( $subtext ) : ?>
                <p class="solution__subtext"><?php echo esc_html( $subtext ); ?></p>
            <?php endif; ?>
        </div>
            ...
            <?php foreach ( $features as $index => $feature ) : ?>
                <div class="solution__card">
                    <div class="solution__card-inner">
                        ...
                        <div>
                            <p class="solution__num"><?php echo esc_html( $feature['num'] ); ?></p>
                            <h3 class="solution__title"><?php echo esc_html( $feature['title'] ); ?></h3>
                            <p class="solution__body"><?php echo esc_html( $feature['body'] ); ?></p>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
