...
$items = [];
for ( $i = 1; $i <= 4; $i++ ) {
    $title = get_theme_mod( "mobiris_trust_item_{$i}_title", $item_defaults[ $i ][0] );
    $desc  = get_theme_mod( "mobiris_trust_item_{$i}_body",  $item_defaults[ $i ][1] );
    if ( $title ) {
        $items[] = [ 'title' => $title, 'body' => $desc ];
    }
}
    ...
    <div class="container">
        <div class="trust__grid">
            ...
            <div class="trust__editorial">
                <?php if ( $label ) : ?>
                    <p class="text-label trust__label"><?php echo esc_html( $label ); ?></p>
                <?php endif; ?>
                <h2 id="trust-headline" class="heading heading--h2 trust__headline">
                    <?php echo esc_html( $headline ); ?>
                </h2>
                <?php if ( $body ) : ?>
                    <p class="trust__body"><?php echo esc_html( $body ); ?></p>
                <?php endif; ?>
                <?php if ( $cta_label ) : ?>
            ...
            <div class="trust__cards">
                <?php foreach ( $items as $index => $item ) : ?>
                    <div class="trust__card">
                        <div class="trust__card-icon" aria-hidden="true">
                            <?php echo $icons[ $index % count( $icons ) ]; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                        </div>
                        <h3 class="trust__card-title"><?php echo esc_html( $item['title'] ); ?></h3>
                        <p class="trust__card-body"><?php echo esc_html( $item['body'] ); ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
