...
<section class="problem" aria-labelledby="problem-headline">
    <div class="container">
        <div class="problem__header">
            <?php if ( $label ) : ?><p class="text-label problem__label"><?php echo esc_html( $label ); ?></p><?php endif; ?>
            <h2 id="problem-headline" class="heading heading--h2 problem__headline"><?php echo esc_html( $headline ); ?></h2>
        </div>
        <div class="problem__grid">
            <?php for ( $i = 1; $i <= 6; $i++ ) :
                $pain = get_theme_mod( "mobiris_problem_pain_{$i}", mobiris_default_pain( $i ) );
                if ( ! $pain ) continue;
            ?>
                <div class="problem__card">
                    <div class="problem__icon"><?php echo $icons[ $i - 1 ]; // phpcs:ignore ?></div>
                    <p class="problem__text"><?php echo esc_html( $pain ); ?></p>
                </div>
            <?php endfor; ?>
        </div>
        <div class="problem__bridge">
            <?php if ( $bridge1 ) : ?><p class="problem__bridge-soft"><?php echo esc_html( $bridge1 ); ?></p><?php endif; ?>
            <?php if ( $bridge2 ) : ?><p class="problem__bridge-bold"><?php echo esc_html( $bridge2 ); ?></p><?php endif; ?>
        </div>
    </div>
</section>
