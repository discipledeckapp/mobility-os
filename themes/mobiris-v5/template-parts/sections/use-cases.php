...
<section id="use-cases" class="usecases" aria-labelledby="usecases-headline">
    <div class="container">
        <div class="usecases__header">
            <?php if ( $label ) : ?><p class="text-label usecases__label"><?php echo esc_html( $label ); ?></p><?php endif; ?>
            <h2 id="usecases-headline" class="heading heading--h2 usecases__headline"><?php echo esc_html( $headline ); ?></h2>
        </div>
        <div class="usecases__grid">
            <?php for ( $i = 1; $i <= 3; $i++ ) :
                $cat  = get_theme_mod( "mobiris_uc_{$i}_category",    $uc_def[$i][0] );
                $head = get_theme_mod( "mobiris_uc_{$i}_headline",    $uc_def[$i][1] );
                $desc = get_theme_mod( "mobiris_uc_{$i}_description", $uc_def[$i][2] );
                $tags = array_filter( array_map( 'trim', explode( ',', get_theme_mod( "mobiris_uc_{$i}_tags", $uc_def[$i][3] ) ) ) );
            ?>
                <div class="usecase-card usecase-card--<?php echo esc_attr( $i ); ?>">
                    <div class="usecase__icon-wrap usecase__icon-wrap--<?php echo esc_attr( $i ); ?>"><?php echo $icons[$i-1]; // phpcs:ignore ?></div>
                    <div>
                        <p class="usecase__category"><?php echo esc_html( $cat ); ?></p>
                        <h3 class="usecase__headline"><?php echo esc_html( $head ); ?></h3>
                        <p class="usecase__description"><?php echo esc_html( $desc ); ?></p>
                    </div>
                    <div class="usecase__tags">
                        <?php foreach ( $tags as $tag ) : ?><span class="usecase__tag"><?php echo esc_html( $tag ); ?></span><?php endforeach; ?>
                    </div>
                </div>
            <?php endfor; ?>
        </div>
